...
- ~~socketio: a driver which sends out the universe via socket.IO as an array (see [demo_socket_client.js](./demo_socket_client.js) as a client example)~~ (removed)
- ~~artnet: driver for EnttecODE~~ (remove)
- ~~bbdmx: driver for [BeagleBone-DMX](https://github.com/boxysean/beaglebone-DMX)~~ (removed)
- ~~dmx4all: driver for DMX4ALL devices like the "NanoDMX USB Interface"~~ (removed)
...
- ~~dmxking-utra-dmx-pro: driver for the DMXKing Ultra DMX pro interface. This driver support multiple universe specify the options with Port = A or B~~ (removed)
